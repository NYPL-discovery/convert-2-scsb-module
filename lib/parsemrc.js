...
module.exports = {
    ...
    if (!field001['001']) {
      ...
      if (record.oclcNumber) {
        ...
        if (field003['003'].length > 1) {
          console.log(`Too many 003 fields added, ${record.bNumber}`)
        }
  ...
  buildItems: function (record) {
    ...
    record.itemData['852'].forEach((item) => {
      if (item['a']) {
        ...
        console.log(`852 missing inumber,${record.bNumber}`)
      }
    })
    ...
    record.itemData['876'].forEach((item) => {
      if (item['a']) {
        ...
        console.log(`876 missing inumber,${record.bNumber}`)
      }
    })
    
    console.log(itemIds)
    console.log('DATA852: ' + data852)
    console.log('DATA876: ' + data876)
    ...
    itemIds.forEach((i) => {
      ...
      if (data852[i] && data876[i]) {
        ...
        if (data852[i].b && data876[i].k) {
          ...
          if (!data852[i].b) {
            console.log(`852|b missing,${i} in ${record.bNumber}`)
          } else {
            console.log(`876|k missing,${i} in ${record.bNumber}`)
          }
          return false
        }
        ...
        if (!data852[i]) {
          console.log(`852 missing,${i} in ${record.bNumber}`)
        } else {
          console.log(`876 missing,${i} in ${record.bNumber}`)
        }
        return false
      }
      ...
      if (!data876[i].p) {
        console.log(`Missing barcode: no 876|p, ${i} in ${record.bNumber}`)
        return false
      } else {
        ...
        if (!data876[i].p.match(regAllDigits)) {
          console.log(`Non numeric barcode found, ${data876[i].p}, item: ${i} in ${record.bNumber}`)
          return false
        }

        // see if we have a duplicate barcode in this file
        if (barcodesUsed.indexOf(parseInt(data876[i].p)) !== -1) {
          console.log(`Duplicate barcode found in single record, ${data876[i].p}, item: ${i} in ${record.bNumber}`)
          return false
        }
  ...
  buildHoldings852and866andItems: function (record, callnumber) {
    ...
    if (record.items[callnumber]) {
      ...
      console.log(`Missing callnumber in items,${callnumber}`)
    }
