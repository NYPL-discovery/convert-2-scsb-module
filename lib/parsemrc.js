...
module.exports = {
  ...
  buildUseRestriction: function (data852, data876, customerCode) {
    // blank by default
    var results = {useRestriction: '', groupDesignation: ''}
    var self = this

    if (['u'].indexOf(data876.o.toString()) !== -1) {
      results.useRestriction = 'Supervised Use'
      results.groupDesignation = self.determineCGD(data852, data876, customerCode)
    } else {
   ...
   */
  determineCGD: function (data852, data876, customerCode) {
    var cgd = ''
    ...
    if (['4', 'a', 'p', 'o'].indexOf(parseInt(data876.o)) !== -1) {
      cgd = 'Private'
      return cgd
    }
