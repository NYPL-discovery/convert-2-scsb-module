...
module.exports = {
  ...
  convertItemsToMiJ: function (items) {
    ...
    items.forEach((item) => {
      ...
      // the 876 should have:
      // |a item number, from the item header in the format .i123456789 (with mod11 check)
      // |d ICODE 2, from fixedFields['60'].value 
      ...
      if (item.fixedFields && item.fixedFields['60'] && item.fixedFields['60'].value) {
        new876['876'].subfields.push({'d': item.fixedFields['60'].value.trim()})
      }
      if (item.barcode) {
        if (item.barcode.trim) item.barcode = item.barcode.trim()
        new876['876'].subfields.push({'p': item.barcode})
      }
      if (item.fixedFields && item.fixedFields['127'] && item.fixedFields['127'].value) {
